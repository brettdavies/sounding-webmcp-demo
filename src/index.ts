...
async function handleSounding(request: Request, opts: SoundingOptions): Promise<Response> {
  ...
  if (path === '/llms.txt') return text(llmsTxt(origin), 'text/plain; charset=utf-8');
  if (path === '/robots.txt') return text(ROBOTS, 'text/plain; charset=utf-8');
  if (path === '/.well-known/mcp/server-card.json') return json(serverCard(origin), 200);
  ...
  if (path === '/') {
    return wantsMarkdown(request) ? markdown(homeMarkdown(origin)) : html(homeHtml(origin), homeHeaders());
  }
...
async function handleMcp(request: Request, complete: boolean): Promise<Response> {
  ...
  if (method === 'initialize') {
    return complete
      ? jsonRpc(id, { protocolVersion: '2025-06-18', capabilities: { tools: {} }, serverInfo: { name: 'sounding', version: '0.1.0' } })
      : jsonRpc(id, { protocolVersion: '2025-06-18', capabilities: { tools: {} } });
  }
...
function json(body: unknown, status: number): Response {
  return new Response(`${JSON.stringify(body, null, 2)}\n`, {
    status,
    headers: { 'Content-Type': 'application/json; charset=utf-8', ...RATE_LIMIT },
  });
}
...
function jsonRpc(id: unknown, result: unknown): Response {
  return new Response(`${JSON.stringify({ jsonrpc: '2.0', id, result })}\n`, {
    status: 200,
    headers: { 'Content-Type': 'application/json; charset=utf-8', ...CORS, ...RATE_LIMIT },
  });
}
